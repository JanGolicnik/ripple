...
{
    glfwInit();
    GLFWwindow* window = glfwCreateWindow(800, 800, "aa", nullptr, nullptr);

    RippleBackendRendererConfig config = ripple_backend_renderer_default_config();
    WGPUSurface surface = glfwGetWGPUSurface(config.instance, window);
    RippleContext ripple_context = ripple_initialize(config);
    ripple_make_active_context(&ripple_context);
    ripple_glfw_register_callbacks(&ripple_context, window);

    i32 prev_width = 0, prev_height = 0;

    Context ctx = create_context(config.device, config.queue);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (width != prev_width || height != prev_height)
        {
            ...
            wgpuSurfaceConfigure(surface, &(WGPUSurfaceConfiguration){
                    .width = max(prev_width, 1),
                    .height = max(prev_height, 1),
                    .format = WGPUTextureFormat_BGRA8UnormSrgb,
        ...
        wgpuInstanceProcessEvents(config.instance);
        WGPUCommandEncoder encoder = wgpuDeviceCreateCommandEncoder(config.device, &(WGPUCommandEncoderDescriptor){ .label = WEBGPU_STR("Command encoder")});
        ...
        {
            wgpuQueueWriteBuffer(config.queue, ctx.shader_data.buffer, 0, &shader_data, sizeof(shader_data));
        ...
        ripple_submit(&ripple_context, width, height, (RippleRenderData){ .texture_view = surface_texture_view, .encoder = encoder } );
        ...
        wgpuQueueSubmit(config.queue, 1, &command);
